...
class Admin extends CI_Controller
{
    ...
    public function edit_profile($admin_id){
        ...
        $data['admin'] = $this->AdminModel->get_profiledata($admin_id);
    ...
    public function do_edit_profile($admin_id){
        ...
        // }
        // else {
        //     $admin_info = $this->AdminModel->get_profiledata($admin_id);
        //     $image_url = $admin_info['image_url'];
        // }
        $result = $this->AdminModel->do_edit_profile($admin_id);
        if (!empty($result)) {
            $this->output->set_output(json_encode(['result' => 1, 'url' => base_url('admin/home'), 'msg' => 'Successfully saved your changes.']));
            return FALSE;
    ...
    public function do_change_password($admin_id){
        ...
        $result = $this->AdminModel->do_check_oldpassword($admin_id);
        
        if (!empty($result)) {
            
            $changed = $this->AdminModel->do_change_passowrd($admin_id);
            if($changed){
                $this->output->set_output(json_encode(['result' => 1, 'url' => base_url('admin/home'), 'msg' => 'Password successfully change.']));
                return FALSE;
            }
            $this->output->set_output(json_encode(['result' => -1, 'msg' => 'Password did not changed successfully.']));
            return FALSE;
